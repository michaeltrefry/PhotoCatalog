...
unsafe extern "C" {
    fn pc_temperature_xy(kelvin: f64, tint: f64, out: *mut WhitePoint) -> i32;
}
        ...
        WhiteBalance::TemperatureTint { kelvin, tint } => {
            ...
            if unsafe { pc_temperature_xy(f64::from(*kelvin), f64::from(*tint), &mut out) } != 0 {
                return Err(RenderError::InvalidInput(
                    "invalid temperature/tint white point".into(),
                ));
            }
            Ok(Some([out.x, out.y]))
        }
...
type Matrix = [[f64; 3]; 3];
const RGB_XYZ: Matrix = [
    [0.4124564, 0.3575761, 0.1804375],
    [0.2126729, 0.7151522, 0.0721750],
    [0.0193339, 0.1191920, 0.9503041],
];
const XYZ_RGB: Matrix = [
...
fn vector(a: Matrix, v: [f64; 3]) -> [f64; 3] {
...
pub(crate) fn adapt_white(pixels: &mut [[f32; 4]], xy: [f64; 2]) {
    ...
    let m = mul(
        XYZ_RGB,
        mul(BRADFORD_INV, mul(diagonal, mul(BRADFORD, RGB_XYZ))),
    );
    for p in pixels {
        let rgb = vector(m, [p[0] as f64, p[1] as f64, p[2] as f64]);
        for i in 0..3 {
            p[i] = rgb[i] as f32;
        }
    }
