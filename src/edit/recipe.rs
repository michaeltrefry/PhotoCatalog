...
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Sharpening { pub amount: f32, pub radius_px: f32 }
#[derive(Debug, Clone, Copy, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct NoiseReduction { pub luminance: f32, pub chroma: f32 }
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum AdjustmentGroup { Geometry, Exposure, WhiteBalance, Tone, Color, Sharpening, NoiseReduction }
...
pub struct ValidatedRecipe { recipe: Recipe, canonical: Vec<u8>, digest: String }
...
impl Default for RecipeV1 {
    fn default() -> Self {
        Self { crop: None, straighten_degrees: 0.0, exposure_ev: 0.0,
            white_balance: WhiteBalance::AsShot, contrast: 0.0, highlights: 0.0,
            shadows: 0.0, saturation: 0.0, vibrance: 0.0,
            sharpening: Sharpening { amount: 0.0, radius_px: 1.0 },
            noise_reduction: NoiseReduction { luminance: 0.0, chroma: 0.0 } }
    }
}
fn checked(name: &str, value: &mut f32, min: f32, max: f32) -> Result<(), RecipeError> {
    if !value.is_finite() || *value < min || *value > max {
        return Err(RecipeError(format!("{name} must be finite in [{min}, {max}]")));
    }
    if *value == 0.0 { *value = 0.0; } // One canonical representation of signed zero.
    Ok(())
}
impl Recipe {
    pub fn validate(&self) -> Result<ValidatedRecipe, RecipeError> {
        ...
        for (name, value) in [("contrast", &mut r.contrast), ("highlights", &mut r.highlights),
            ("shadows", &mut r.shadows), ("saturation", &mut r.saturation), ("vibrance", &mut r.vibrance)] {
            checked(name, value, -1.0, 1.0)?;
        }
        checked("sharpening.amount", &mut r.sharpening.amount, 0.0, 2.0)?;
        checked("sharpening.radius_px", &mut r.sharpening.radius_px, 0.1, 10.0)?;
        checked("noise_reduction.luminance", &mut r.noise_reduction.luminance, 0.0, 1.0)?;
        checked("noise_reduction.chroma", &mut r.noise_reduction.chroma, 0.0, 1.0)?;
        if let WhiteBalance::TemperatureTint { kelvin, tint } = &mut r.white_balance {
            if !(2000..=50000).contains(kelvin) { return Err(RecipeError("white balance kelvin must be 2000..=50000".into())); }
            checked("white_balance.tint", tint, -150.0, 150.0)?;
        }
        if let Some(crop) = &mut r.crop {
            for (name,value) in [("crop.left", &mut crop.left), ("crop.top", &mut crop.top),
                ("crop.right", &mut crop.right), ("crop.bottom", &mut crop.bottom)] { checked(name,value,0.0,1.0)?; }
            if crop.left >= crop.right || crop.top >= crop.bottom { return Err(RecipeError("crop must have positive width and height".into())); }
        }
        ...
        Ok(ValidatedRecipe { recipe, canonical, digest })
    }
    ...
    pub fn copy_groups_from(&self, source: &Recipe, groups: &[AdjustmentGroup]) -> Result<ValidatedRecipe, RecipeError> {
        ...
        for group in groups {
            match group {
                AdjustmentGroup::Geometry => { t.crop = s.crop; t.straighten_degrees = s.straighten_degrees; }
                AdjustmentGroup::Exposure => t.exposure_ev = s.exposure_ev,
                AdjustmentGroup::WhiteBalance => t.white_balance = s.white_balance.clone(),
                AdjustmentGroup::Tone => { t.contrast = s.contrast; t.highlights = s.highlights; t.shadows = s.shadows; }
                AdjustmentGroup::Color => { t.saturation = s.saturation; t.vibrance = s.vibrance; }
...
mod tests {
    ...
    fn canonical_roundtrip_zero_and_finite_validation() {
        let mut r = RecipeV1::default(); r.exposure_ev = -0.0;
        ...
        r.shadows = f32::NAN; assert!(Recipe::V1(r).validate().is_err());
    }
    #[test]
    fn unknown_versions_fields_and_invalid_geometry_rejected() {
        let mut value = serde_json::to_value(Recipe::default()).unwrap();
        value["version"] = serde_json::json!("2"); assert!(serde_json::from_value::<Recipe>(value.clone()).is_err());
        value["version"] = serde_json::json!("1"); value["settings"]["healing"] = serde_json::json!(true);
    ...
    fn copy_keeps_unselected_groups_and_changes_canonical_identity() {
        let mut source = RecipeV1::default(); source.exposure_ev = 1.0; source.contrast = 0.5;
        let target = Recipe::default();
        let copied = target.copy_groups_from(&Recipe::V1(source), &[AdjustmentGroup::Exposure]).unwrap();
        assert_eq!(copied.settings().exposure_ev, 1.0); assert_eq!(copied.settings().contrast, 0.0);
        assert_ne!(copied.digest(), target.validate().unwrap().digest());
        assert_eq!(target.copy_groups_from(copied.recipe(), &[]).unwrap(), target.validate().unwrap());
    }
}
