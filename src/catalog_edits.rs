...
CREATE INDEX edit_variants_page ON edit_variants(asset_id,sequence);
...
CREATE TABLE edit_copy_items(
 job TEXT NOT NULL REFERENCES edit_copy_jobs(id), sequence INTEGER NOT NULL,
 asset_id TEXT NOT NULL, variant_id TEXT NOT NULL, expected_revision INTEGER NOT NULL,
 state TEXT NOT NULL CHECK(state IN ('pending','applied','conflict','incompatible')),
 applied_revision INTEGER, error TEXT,
 PRIMARY KEY(job,sequence), UNIQUE(job,asset_id,variant_id));
CREATE INDEX edit_copy_pending ON edit_copy_items(job,sequence) WHERE state='pending';
";
...
impl VariantKey {
    ...
    pub fn validate(&self) -> Result<()> {
            ...
            !self.asset_id.is_empty() && self.asset_id.len() <= 256,
            "asset identity length"
        );
        ensure!(
            !self.variant_id.is_empty() && self.variant_id.len() <= 256,
            "variant identity length"
        );
        Ok(())
    }
}
...
fn canonical(recipe: &Recipe) -> Result<(Vec<u8>, String)> {
    ...
    let bytes = validated.canonical_bytes().to_vec();
        ...
        "recipe exceeds durable document limit"
    );
...
fn read_recipe(bytes: &[u8], digest: &str) -> Result<Recipe> {
        ...
        bytes.len() <= MAX_RECIPE_BYTES,
        "stored recipe exceeds document limit"
    );
    let recipe: Recipe = serde_json::from_slice(bytes)?;
    let (canonical, actual) = canonical(&recipe)?;
    ensure!(
        canonical == bytes && actual == digest,
        "stored recipe identity mismatch"
    );
    Ok(recipe)
}
...
fn insert_variant(tx: &Connection, key: &VariantKey, label: &str, recipe: &Recipe) -> Result<()> {
    ...
    tx.execute(
        "INSERT INTO edit_variants(asset_id,id,label,revision) VALUES(?1,?2,?3,0)",
        params![key.asset_id, key.variant_id, label],
    )?;
    ...
    tx.execute(
        "UPDATE edit_variants SET cursor=?1 WHERE asset_id=?2 AND id=?3",
        params![node, key.asset_id, key.variant_id],
    )?;
    Ok(())
}
...
fn record_change(
    ...
    let provenance = serde_json::to_string(provenance)?;
        ...
        provenance.len() <= MAX_RECIPE_BYTES,
        "edit provenance document limit"
    );
...
fn save(
    ...
    let current = materialize(tx, key)?;
        ...
        expected >= 0 && current.revision == expected,
        "edit revision changed"
    );
    let revision = current
        .revision
        .checked_add(1)
        .context("edit revision exhausted")?;
    ...
    tx.execute(
        "UPDATE edit_variants SET revision=?1,cursor=?2,redo=NULL WHERE asset_id=?3 AND id=?4",
        params![revision, node, key.asset_id, key.variant_id],
    )?;
...
impl Catalog {
    ...
    pub fn edit_variants(
        &self,
        asset: &str,
        after: i64,
        limit: usize,
    ) -> Result<Vec<(i64, VariantView)>> {
        ensure!(
            (1..=200).contains(&limit) && after >= 0,
            "variant page bounds"
        );
        let tx = self.db.unchecked_transaction()?;
    ...
    pub fn create_edit_variant(
        ...
        record_change(
            ...
            value.cursor,
            value.cursor,
            &serde_json::json!({"source":source,"source_revision":expected_revision,"recipe_digest":input.recipe_digest}),
        )?;
    ...
    fn move_edit_cursor(
        ...
        let (node, next_redo) = if redo {
            ...
            let parent: Option<i64> = tx.query_row("SELECT parent FROM edit_recipe_nodes WHERE id=?1 AND asset_id=?2 AND variant_id=?3", params![node,key.asset_id,key.variant_id], |r|r.get(0))?;
                ...
                parent == Some(current.cursor),
                "redo lineage does not match current recipe"
            );
            (node, next)
        } else {
        ...
        tx.execute(
            "UPDATE edit_variants SET revision=?1,cursor=?2,redo=?3 WHERE asset_id=?4 AND id=?5",
            params![revision, node, next_redo, key.asset_id, key.variant_id],
        )?;
        record_change(
            &tx,
            key,
    ...
    pub fn edit_history(
        ...
        key.validate()?;
            ...
            (1..=200).contains(&limit) && after_revision >= -1,
            "edit history page bounds"
        );
    ...
    pub fn edit_render_identity(&self, key: &VariantKey) -> Result<EditRenderIdentity> {
        ...
        Ok(EditRenderIdentity {
            ...
            recipe_digest: variant.recipe_digest,
        })
    }

    pub fn with_edit_identity<T>(
        &mut self,
    ...
    pub(crate) fn with_edit_transaction<T>(
        &mut self,
        expected: &EditRenderIdentity,
        priority: Priority,
        attach: impl FnOnce(&Transaction<'_>) -> Result<T>,
    ) -> Result<Option<T>> {
