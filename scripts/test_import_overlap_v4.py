...
import observe_import_native_v4 as observer
from evaluate_import_overlap_v4 import COUNTERS, evaluate
...
def fixture():
    ...
    rows = [{'kind': 'identity', 'protocol': observer.PROTOCOL, 'clock': observer.CLOCK, 'profile': 'import_v1',
            ...
            {'kind': 'summary', 'protocol': observer.PROTOCOL, 'clock': observer.CLOCK, 'fatal_errors': 0, 'root_same_birth': True, 'executable_unchanged': True,
...
class EvaluatorTests(unittest.TestCase):
    def test_exact100_boundary_and_budget(self):
        ...
        self.assertEqual(result['protocol'], observer.PROTOCOL)
    ...
    def test_protocol3_observer_is_not_reinterpreted_as_protocol4(self):
        values = fixture()
        values[1][0]['protocol'] = 3
        values[1][-1]['protocol'] = 3
        with self.assertRaisesRegex(ValueError, 'Observer clock/profile'):
            evaluate(*values)
    ...
    def test_cli_hashes_inputs_and_refuses_overwrite(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            command = [sys.executable, str(Path(__file__).with_name('evaluate_import_overlap_v4.py'))]
...
class ObserverTests(unittest.TestCase):
    ...
    def test_invalid_preflight_writes_error_and_summary(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            result = subprocess.run([sys.executable, str(Path(__file__).with_name('observe_import_native_v4.py')),
    ...
    def test_declared_role_projection_requires_exact_targeted_proofs(self):
        _, rows, declaration = fixture()
        processes = rows[1]['processes']
        expected = {role: [{'pid': declaration[f'{role}_pid'],
                            'birth_unix_s': declaration[f'{role}_birth_unix_s']}]
                    for role in observer.ROLES}
        self.assertEqual(observer.declared_role_owners(declaration, processes), expected)
        missing = copy.deepcopy(processes); missing.pop('desktop')
        with self.assertRaisesRegex(ValueError, 'Missing declared process proof'):
            observer.declared_role_owners(declaration, missing)
        mismatched = copy.deepcopy(processes); mismatched['filesystem']['pid'] += 1
        with self.assertRaisesRegex(ValueError, 'Missing/mismatched'):
            observer.declared_role_owners(declaration, mismatched)

    def test_probe_reads_only_declared_processes_not_unrelated_gui_children(self):
        declaration = fixture()[2]
        root = Mock(); desktop = Mock(); filesystem = Mock()
        for role, process in (('root', root), ('desktop', desktop), ('filesystem', filesystem)):
            process.exe.return_value = declaration['executable']
            process.cmdline.return_value = [declaration['executable']] + ([] if role == 'root' else [observer.ROLES[role]])
            process.ppid.return_value = 1 if role == 'root' else declaration['root_pid']
        root.children.side_effect = AssertionError('global child enumeration is outside protocol 4')
        unrelated = Mock()
        unrelated.cmdline.side_effect = observer.psutil.AccessDenied(45)
        process_by_pid = {42: root, 43: desktop, 44: filesystem, 45: unrelated}
        requested = []

        def targeted(pid):
            requested.append(pid)
            return process_by_pid[pid]

        with patch.object(observer.psutil, 'Process', side_effect=targeted), \
             patch.object(observer, 'same_birth', return_value=True), \
             patch.object(observer, 'executable_identity', return_value=declaration['executable_identity']), \
             patch.object(observer, 'file_identity', side_effect=[[1, 3], [1, 4], [1, 3], [1, 4]]), \
             patch.object(observer, 'lock_contended', side_effect=[True, True]), \
             patch.object(observer, 'import_lock_holder', return_value={'holder': True}):
            proof = observer.positive_probe(declaration)
        self.assertEqual(requested, [42, 43, 44, 42, 43, 44])
        root.children.assert_not_called()
        unrelated.cmdline.assert_not_called()
        self.assertEqual(proof['role_owners'], {
            role: [{'pid': declaration[f'{role}_pid'],
                    'birth_unix_s': declaration[f'{role}_birth_unix_s']}]
            for role in observer.ROLES
        })

    def test_declared_process_cmdline_access_denial_remains_fatal(self):
        declaration = fixture()[2]
        filesystem = Mock()
        filesystem.exe.return_value = declaration['executable']
        denial = observer.psutil.AccessDenied(declaration['filesystem_pid'])
        denial.__cause__ = PermissionError(
            13, 'force permission denied (originated from sysctl(KERN_PROCARGS2) -> EINVAL)')
        filesystem.cmdline.side_effect = denial
        with patch.object(observer.psutil, 'Process', return_value=filesystem), \
             patch.object(observer, 'same_birth', return_value=True), \
             patch.object(observer, 'executable_identity', return_value=declaration['executable_identity']):
            with self.assertRaises(observer.psutil.AccessDenied) as raised:
                observer.process_proof(declaration, 'filesystem')
        self.assertIs(raised.exception, denial)

    @unittest.skipUnless(sys.platform == 'darwin', 'Darwin process churn conformance')
    def test_darwin_short_lived_children_do_not_affect_declared_binding(self):
        # Exercise the real Darwin child/cmdline lifecycle race, while the v4
        # proof remains a pure projection of already-proved declared roles.
        _, rows, declaration = fixture()
        processes = rows[1]['processes']
        expected = rows[1]['role_owners']
        lifecycle_errors = []
        churn_errors = []
        ...
        def churn():
            ...
            except BaseException as error:  # Report background failures in the test thread.
        ...
        while worker.is_alive() and time.monotonic() < deadline:
            for child in observer.psutil.Process(os.getpid()).children():
                try:
                    child.cmdline()
                except (observer.psutil.NoSuchProcess, observer.psutil.ZombieProcess,
                        observer.psutil.AccessDenied) as error:
                    lifecycle_errors.append(error)
            self.assertEqual(observer.declared_role_owners(declaration, processes), expected)
            scans += 1
        stop.set()
        worker.join(timeout=1)
        self.assertFalse(worker.is_alive(), 'bounded child churn exceeded deadline')
        self.assertFalse(churn_errors)
        self.assertGreater(scans, 0)
        self.assertTrue(all(isinstance(error, (observer.psutil.NoSuchProcess,
                                               observer.psutil.ZombieProcess,
                                               observer.psutil.AccessDenied))
    ...
    def test_probe_rechecks_after_lsof_and_contention_loss(self):
        declaration = fixture()[2]
        processes = fixture()[1][1]['processes']
        with patch.object(observer, 'executable_identity', return_value=declaration['executable_identity']), \
             ...
             patch.object(observer, 'process_proof', side_effect=lambda binding, role: processes[role]), \
    ...
    def test_probe_rejects_declared_process_change_after_lsof(self):
        declaration = fixture()[2]
        processes = fixture()[1][1]['processes']
        changed = copy.deepcopy(processes['filesystem'])
        changed['parent_pid'] += 1
        proofs = [processes['root'], processes['desktop'], processes['filesystem'],
                  processes['root'], processes['desktop'], changed]
        with patch.object(observer, 'executable_identity', return_value=declaration['executable_identity']), \
             patch.object(observer, 'file_identity', side_effect=[[1, 3], [1, 4], [1, 3], [1, 4]]), \
             patch.object(observer, 'process_proof', side_effect=proofs), \
             patch.object(observer, 'lock_contended', return_value=True), \
             patch.object(observer, 'import_lock_holder', return_value={}):
            with self.assertRaisesRegex(ValueError, 'Process identity changed'):
                observer.positive_probe(declaration)
