...
def declared_role_owners(binding, processes):
    """Project the exact C/F bindings after their targeted proofs succeed."""
    require(set(processes) == {'root', *ROLES}, 'Missing declared process proof')
    found = {
        role: [{'pid': processes[role]['pid'], 'birth_unix_s': processes[role]['birth_unix_s']}]
        for role in ROLES
    }
    expected = {role: [{'pid': binding[f'{role}_pid'], 'birth_unix_s': binding[f'{role}_birth_unix_s']}] for role in ROLES}
    require(found == expected, 'Missing/mismatched declared GUI catalog-worker owners')
    return found
...
def positive_probe(binding):
    ...
    owners = declared_role_owners(binding, processes)
    ...
    rechecked = {role: process_proof(binding, role) for role in ('root', 'desktop', 'filesystem')}
    require(rechecked == processes, 'Process identity changed after lsof')
    require(declared_role_owners(binding, rechecked) == owners, 'Declared GUI role binding changed after lsof')
    if not lock_contended(lock, tuple(binding['lock_identity'])):
        return None
...
def observe(binding, emit, seconds=600, interval=.2):
    ...
    emit({'kind': 'identity', 'protocol': PROTOCOL, 'clock': CLOCK, 'profile': 'import_v1',
    ...
    try:
        ...
        emit({'kind': 'error', 'operation': 'final_identity', 'monotonic_ns': time.monotonic_ns(),
              'error': f'{type(error).__name__}: {error}', 'exception_chain': exception_evidence(error)})
    summary = {'kind': 'summary', 'protocol': PROTOCOL, 'clock': CLOCK, 'fatal_errors': fatal,
    ...
    with args.output.open('x') as output:
        ...
        try:
            ...
            emit({'kind': 'error', 'operation': 'preflight', 'monotonic_ns': time.monotonic_ns(),
                  'error': f'{type(error).__name__}: {error}', 'exception_chain': exception_evidence(error)})
            emit({'kind': 'summary', 'protocol': PROTOCOL, 'clock': CLOCK, 'fatal_errors': 1, 'usable_segments': 0})
            return 2
