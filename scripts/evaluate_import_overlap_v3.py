...
from observe_import_native_v3 import BINDINGS, CLOCK, ROLES, declaration_digest, require, validate_declaration
...
def native_segments(rows, declaration):
    ...
    require(identity['protocol'] == summary['protocol'] == 3 and identity['clock'] == summary['clock'] == CLOCK
    ...
    for segment in segments:
        ...
        for row in positives:
            ...
            require(row['role_owners'] == {role: [{'pid': declaration[f'{role}_pid'], 'birth_unix_s': declaration[f'{role}_birth_unix_s']}] for role in ROLES}, 'Ambiguous GUI role owners')
...
def evaluate(receipt, rows, declaration):
    ...
    result = {'protocol': 3, 'run_id': receipt['run_id'], 'import_id': declaration['import_id'], 'cohort': expected,
