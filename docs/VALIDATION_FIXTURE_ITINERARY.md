...
3. Inspect all four `inputs/lightroom-catalogs` candidates while the bound `inputs/originals` root is still present. In **Families**, use **Record explicit family choice** with a meaningful reason for the current member of each family (`2014-v13-2.lrcat` and `2015-v13.lrcat`). Reread and assemble the current **Families** report, confirm those two persisted choices, and stage the complete two-family review; the older candidates are unselected members of those families, not separate family decisions. Compare dry-run source counts with `expected-reconciliation.json`, and migrate into the same catalog used in step 2. Repeat/resume once. Require the coordinator-qualified destination counts, zero excluded-catalog records, stable shared-path handling, both virtual copies, five collections, four retained catalog packets, and explicit compatibility status.
