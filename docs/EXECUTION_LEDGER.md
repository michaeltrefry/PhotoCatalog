...
sc-22836–sc-22842 are verified Done. S8's full headless qualification and corrected PR #9 CI passed; the merged-main run exposed a separate worker-lock cleanup defect now under correction. S9 has an adopted inspection checkpoint and locally validated integration candidate. Once local corrective checks finish and fresh resource admission passes, inspection may run alongside remote CI: its only story prerequisite is completed S4, and reviewed worker cleanup changes require no new performance campaign. Native builds and large-I/O/measurement work still use one owned lane. Live Shortcut remains authoritative; this checkpoint was reconciled on 2026-09-10. Sections below the current ledger retain historical checkpoints.
...
| sc-22843 | In Review | [PR #9](https://github.com/michaeltrefry/PhotoCatalog/pull/9), merged f6d19dc; codex/sc-22843-worker-lease-fix; [qualification report](EDIT_QUALIFICATION_RESULTS.md) | Reviewed 533 cases/249 numerical configurations PASS; corrected PR CI34535465873 all green. Main CI34537128387 exposed Linux duplicate-descriptor lock release; deterministic export/preview regressions fail before and pass after correction | Review and validate worker-lease correction, corrective PR CI, merge and main CI |
| sc-22844 | In Progress | Frozen v6 adoption; codex/sc-22844-s8-integration atb228bd7 | 15 captures/14 completed outcomes/9,739,671 retained rows reconciled; active member24 verified pages. Integrated412 native/71 Python checks and final35 focused Lightroom tests PASS; final source review PASS | After local native lane release and fresh admission, resume main while remote CI runs; finish auxiliary/path/packet phases and family ambiguity review before migration |
