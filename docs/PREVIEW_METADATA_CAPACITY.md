...
`Failure::new`-bounded 4096-byte message. The report charges old/new bounded
strings during adoption for `W+1` render/read transports. Encoded delivery is
phase-exclusive with those transports and fits the same envelope. Relay fault
backing is split by its source owner: parent latch, child latch, two child call
outcomes, admission query, store query, native pending query and retained native
result.
