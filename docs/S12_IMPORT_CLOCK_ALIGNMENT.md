...
The inherited requirement is durable acknowledgment p95 **≤100 ms while background import runs**. This protocol measures exactly **100 predeclared rating or edit inputs**, all successful, with no latency-based selection. It does not change that budget. Historical export protocol-2 and import observer protocol-3 campaigns retain their original verdicts; in particular a prior PARTIAL or invalid run cannot become an import pass under protocol 4.
...
The GUI owns C (`--catalog-desktop-worker`) and F (`--catalog-filesystem-worker`) as **direct siblings**. They are not a GUI→C→F process chain: the single Tauri setup constructs one `DesktopBridge`, then `src/application/desktop/startup.rs` creates retained F before `process::Owner::spawn` creates C. A fresh one-catalog session is mandatory. Every positive probe binds the declared root/C/F exact PIDs and births, exact C/F role arguments, direct ancestry, executable path and executable object before and after the lock query.

Protocol 4 makes only that declared-role claim. It does not enumerate unrelated direct GUI children, inspect their command lines, or claim a complete runtime inventory of every process with role-looking arguments. Transient preview and platform helpers therefore cannot invalidate the measurement merely by exiting during an unrelated command-line read. This is justified by the reviewed construction invariant above and remains fail-closed for any declared root/C/F identity, role, ancestry, executable or lifecycle failure. The evaluator independently requires those exact declared bindings on every positive row.
