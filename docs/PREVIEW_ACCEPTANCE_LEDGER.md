...
Checkpoint: source f9b29050834d79bc03bf6a13cfd59c32560779f1; parent source review
PASS. S6 remains **In Progress**. This ledger reconciles the three sc-22841
...
| --- | --- | --- |
| S7 integration | After reviewed S7 merge, integrate shared foreground/background catalog writer admission. Review S6 import reserve/publication/recovery transaction boundaries for the shared gate and avoid nested admission. | Current correctness predates this change. No claim that native-worker priority alone prevents catalog-writer starvation. |
| Integrated donor/schema reconciliation | Current source/protocol explicitly requires schema 4. Reconcile with merged S7 schema 5 using an explicitly reviewed donor/preparation protocol before running. | **Execution blocked until reconciled.** Never let future Catalog::open migrate a schema-4 fixture during measurement. No original SQLite connections. |
| Current-head correctness and build freeze | Run the eight source-ready integrated tests plus affected shared fixture/layout/navigation tests, then complete relevant Rust/Python/fmt/Clippy gates. Freeze clean whole-source archive, actual release app/probes, native dependency versions, storage and input receipts. | f9b2905 has formatting/syntax/source review only; no runtime test or campaign results. Rebuild the package after checkout switches. |
