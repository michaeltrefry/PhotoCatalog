...
Status: **Original and supplemental comparisons complete; corrected-query timing verified; native integration pending.** SQLite remains provisional. DuckDB at 1024 MiB passes the numerical gates across all scales, but its measured deep-page scan work grows with catalog size. None of SQLite's original SQL profiles qualifies. Equivalent SQLite query corrections show constant VM work at all tested scales; their conditional 256 MiB timing experiment passes every fixed gate at all three scales. No production backend is selected yet.
...
The original Rust 10M probe preserves identity/metadata across restart and records rating/edit p95 of 99.544/99.189 ms with only 50 samples each and less than 1 ms budget headroom. That evidence belongs to the frozen probe, not a later integrated implementation. Plan presence is separate from bounded work. Read-only diagnostics now cover 50%, 90%, and exact frozen iteration-9 cursors at all three scales. Independent review reconciled all 7,200 SQLite records: the corrected page/rating queries use 2,410/2,413 VM steps and no sort in every case. DuckDB's 90.5% page scans 340,153 / 1,020,014 / 1,531,215 rows at 1M/5M/10M. The bundled Rust diagnostic also verifies identical records and constant 2,410/2,413 VM steps at every tested scale/cursor, with no sort. Its five source-preservation/contract tests pass. All 39 small Python contract tests pass locally; hosted CI at 5a1f13d passed the previous 38-test suite on the exact committed artifact.
...
No production selection, compatibility migration, or acceptance conclusion is recorded yet. After corrected-query timing and native integration are verified, this document will state the selected backend, rationale, measured tradeoffs, remaining limitations, and the evidence that existing skeleton identities and data survive the integration.
