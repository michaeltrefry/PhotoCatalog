...
These tests cover actual child SQL dispatch, recovery dispatch, matching baseline hashes and parameter schedules, rejection of errors/incomplete samples/incorrect identities and oversized warm/fresh RSS, acceptance of valid mixed RSS above 4 GiB while rejecting missing/invalid measurements, and preservation of existing output. They are small synthetic correctness tests, not scale or performance evidence. No tests were run while writing this protocol because the original supplemental campaign held the timing lane.
