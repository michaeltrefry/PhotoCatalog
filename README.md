...
The foundation provides a UI-independent library and CLI with SQLite metadata storage and provisional JPEG thumbnails. The [database decision](docs/BACKEND_DECISION.md) records the measured configuration and its limits; preview formats retain their own benchmark story. The image pipeline adds full-resolution RAW/DNG development and common raster/PSD composite decoding, with explicit color/precision provenance and compatibility limits. See [image pipeline](docs/IMAGE_PIPELINE.md) for the supported variants and native build requirements. XMP source retention, conflict review, catalog-only metadata edits, and controlled export are described in [metadata workflow](docs/XMP_METADATA.md). Organization and photographic search provide durable keywords, ratings, flags, labels, collections, snapshot paging and resumable batches through the [core/CLI workflow](docs/ORGANIZATION_SEARCH.md). The [organization performance report](docs/ORGANIZATION_PERFORMANCE_RESULTS.md) records all three catalog scales and remaining delivery gates. Versioned editing recipes, variant-aware previews and full-original batch export are described in the [editing workflow](docs/EDITING_AND_EXPORT.md); S8 qualification remains in progress. The desktop interface remains tracked work.
...
See [requirements](docs/REQUIREMENTS.md), [implementation plan](docs/IMPLEMENTATION_PLAN.md), and [execution ledger](docs/EXECUTION_LEDGER.md). The live Shortcut epic and stories are the operational source of truth. Relinking, preview scheduling and organization/search have merged with platform validation. Editing/export qualification, Lightroom migration, backup/restore and the desktop interface remain open tracked requirements in the epic.
