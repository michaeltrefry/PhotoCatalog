...
import { selectMetadataModel, type MetadataModelDestination, type SelectedMetadataModel } from '../metadataModelSelection';
import './metadata.css';
...
function Observations({ catalog, identity, inspect, chooseModel }: { catalog: string; identity: ImageIdentity; inspect: Inspect; chooseModel: (model: SelectedMetadataModel, destination: MetadataModelDestination) => void }) {
  const [selected, setSelected] = useState<{ id: string; source: string; current: boolean } | null>(null); const [view, setView] = useState<'models' | 'packets'>('packets');
  return <><Pages catalog={catalog} kind="observations" request={after => ({ command: 'observations', args: { identity, after, limit: 20 } })}>{row => <>
    ...
    <button aria-pressed={selected?.id === row.id} onClick={() => setSelected({ id: row.id, source: row.source, current: row.current })}>Inspect packets and models</button>
  </>}</Pages>{selected && <section><h3>Retained observation {selected.id}</h3><p className="hint">Source {selected.source} · {selected.current ? 'current observation' : 'retained earlier observation'}</p><nav aria-label="Observation data"><button aria-pressed={view === 'packets'} onClick={() => setView('packets')}>Original packets</button><button aria-pressed={view === 'models'} onClick={() => setView('models')}>Parsed models</button></nav>
    {view === 'packets' ? <Pages key={`packets:${selected.id}`} catalog={catalog} kind="packets" request={after => ({ command: 'packets', args: { identity, observation: selected.id, after, limit: 20 } })}>{row => <><Value label="Packet descriptor" value={row.descriptor} identity={identity} inspect={inspect} /><p>{row.bytes} bytes</p><button onClick={() => inspect('Original packet', { command: 'blob_chunk', args: { key: identity.key, source: { kind: 'packet', observation: selected.id, ordinal: row.ordinal }, offset: '0', length: 4096 } })}>Inspect retained packet bytes</button></>}</Pages>
      : <Pages key={`models:${selected.id}`} catalog={catalog} kind="models" request={after => ({ command: 'models', args: { identity, observation: selected.id, after, limit: 20 } })}>{row => <><p><strong>Model {row.id}</strong> · ordinal {row.ordinal} · {row.bytes} bytes</p><p className="hint">Source {selected.source} · observation {selected.id} · BLAKE3 {row.blob_hash}</p>{(['descriptor', 'projection', 'error'] as const).map(name => <Value key={name} label={name} value={row[name]} identity={identity} inspect={inspect} />)}{row.error !== null && <p className="metadata-conflict">This model has a retained parse error and cannot be selected as a complete base.</p>}<div className="button-group"><button onClick={() => inspect('Retained model', { command: 'blob_chunk', args: { key: identity.key, source: { kind: 'model', id: row.id }, offset: '0', length: 4096 } })}>Inspect retained model bytes</button><button disabled={row.error !== null} onClick={() => chooseModel(selectMetadataModel(row, selected), 'edit')}>Use for metadata edit</button><button disabled={row.error !== null} onClick={() => chooseModel(selectMetadataModel(row, selected), 'sidecar')}>Use for sidecar</button></div></>}</Pages>}
  </section>}</>;
}
  ...
  return <section><p>Browse the imported Lightroom record, its linked history, and retained settings. Retention does not mean Adobe rendering equivalence.</p>
    ...
    {page && <><p>Record classification: {page.row.classification}</p><p className="hint">Retained relationship/index coverage {page.coverage_complete ? 'complete' : 'incomplete'} · keys {page.keys_complete ? 'complete' : 'incomplete'} · Adobe rendering equivalence: {page.adobe_rendering_equivalent ? 'reported' : 'not established'}.</p>
      ...
      <details open={settingsOpen} onToggle={e => { const open = e.currentTarget.open; setSettingsOpen(open); if (!open) { setSettingsPath('[]'); setInterpret(null); } }}><summary>Adobe settings interpretation</summary>{settingsOpen && <><p>Choose a retained row column and its settings path. The original field remains accessible even when interpretation is unavailable.</p>
        <ImportedColumns key={`columns:${page.anchor_json}`} catalog={catalog} variant={variant} anchor={page.anchor_json} selected={column} choose={setColumn} /><p>Selected column: {column || 'Choose a column above'}</p><SettingsPath key={`settings:${page.anchor_json}`} onChange={setSettingsPath} /><button disabled={!column} onClick={() => setInterpret({ column, path: settingsPath })}>Inspect settings</button>
...
export function MetadataPanel({ catalog, variant, filename, mutate, onClose }: { catalog: string; variant: VariantKey; filename: string; mutate: Mutation; onClose: () => void }) {
  ...
  const [selectedModel, setSelectedModel] = useState<SelectedMetadataModel | null>(null);
  const key = JSON.stringify(variant);
  useEffect(() => { const abort = new AbortController(); setIdentity(null); setField(null); setBytes(null); setSelectedModel(null); setError(''); void metadata(catalog, { command: 'identity', args: { key: variant } }, 'identity', abort.signal).then(value => { if (!abort.signal.aborted) setIdentity(value); }).catch(e => { if (!abort.signal.aborted) setError(errorText(e)); }); return () => abort.abort(); }, [catalog, key, epoch]);
  ...
  return <Dialog title={`Metadata · ${filename}`} onClose={onClose}><div className="metadata-panel"><nav aria-label="Metadata sections">{tabs.map(([id, title]) => <button key={id} aria-pressed={tab === id} onClick={() => { setTab(id); setBytes(null); }}>{title}</button>)}</nav><button onClick={() => setEpoch(v => v + 1)}>Refresh current metadata</button>
    ...
    {identity && <div key={JSON.stringify(identity)}>
      ...
      {tab === 'edit' && <MetadataEditor catalog={catalog} identity={identity} selectedModel={selectedModel} changed={() => setEpoch(v => v + 1)} />}
      {tab === 'sidecar' && <SidecarPanel catalog={catalog} identity={identity} selectedModel={selectedModel} />}
      ...
      {tab === 'observations' && <Observations catalog={catalog} identity={identity} inspect={inspect} chooseModel={(model, destination) => { setSelectedModel(model); setBytes(null); setTab(destination); }} />}
