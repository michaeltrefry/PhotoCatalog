...
export function LightroomPanel({controller:c,open,onClose}:{controller:Controller;open:boolean;onClose:()=>void}) {
  ...
  const [queryKind,setQueryKind]=useState<Query['kind']>('Families'),[collection,setCollection]=useState<Extract<Query,{kind:'SelectionPage'}>['collection']>('Families');
  ...
  return <Dialog title="Lightroom inspection" onClose={onClose}><div className="lightroom-panel">
    ...
    <Section title="Browse retained inspection evidence">
      ...
      <label>Evidence view<select value={queryKind} onChange={e=>setQueryKind(e.target.value as Query['kind'])}>{(['Families','CaptureManifest','Rows','Report','Paths','Issues','Packets','PacketBytes','MetadataConflicts','GlobalIdConflicts','PathCollisions','SelectionSummary','SelectionPage'] as const).map(kind=><option key={kind}>{kind}</option>)}</select></label>
