...
import { metadataWrite, metadataWriteOperationError, metadataWriteTerminal, type Action, type ImageIdentity, type Operation, type Review, type Status } from '../metadataWrite';
...
export function useMetadataWrite(catalog:string, identity:ImageIdentity|null) {
  ...
  const start=useCallback(async(action:Action)=>{setBusy(true);setError('');try{const admitted=await metadataWrite(catalog,{command:'start',args:{attempt:attempt(),action}},'admitted');for(let n=0;n<120;n++){const value=await metadataWrite(catalog,{command:'status',args:{operation:admitted.operation}},'status');current.current=value;setStatus(value);if(value.operation?.id===admitted.operation&&metadataWriteTerminal(value.operation)){if(value.operation.phase!=='complete')throw new Error(metadataWriteOperationError(value.operation));return value;}await new Promise(resolve=>setTimeout(resolve,100));}throw new Error('Metadata operation status did not settle.');}catch(e){setError(errorText(e));throw e;}finally{setBusy(false);}},[catalog]);
