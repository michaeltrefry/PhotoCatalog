...
export async function prepareCaptureArtifacts(capture:SelectedCapture,directory:NativePath,maximumBytes:string,openMs:string,signal?:AbortSignal,retained?:(value:RetainedArtifactPreparation|null)=>void,send:(request:ArtifactPreparationRequest,signal?:AbortSignal)=>Promise<ArtifactPreparationReply|null>=artifactPreparation,received?:(value:ArtifactReceipt)=>void):Promise<ArtifactReceipt[]> {
  ...
  try{
    ...
    const members=BigInt(decimal(begun.members,'Artifact member count',1n,4096n)),result:ArtifactReceipt[]=[];
    for(let index=0n;index<members;index++){
      const reply=await send({action:'member',session,member_index:index.toString()},signal);
      if(!reply||reply.kind!=='prepared'||reply.session!==session||reply.member_index!==index.toString()||!uuid(reply.receipt))throw new Error('Prepared artifact receipt identity differs.');
      const prepared={receipt:reply.receipt};result.push(prepared);received?.(prepared);
    }
    return result;
...
export async function discardArtifactReceipt(receipt:string):Promise<void>{if(!uuid(receipt))throw new Error('Prepared artifact receipt is invalid.');const reply=await artifactPreparation({action:'discard_receipt',receipt});if(reply!==null)throw new Error('Unexpected artifact receipt discard response.');}
...
export function approvalDraft(value:{reviewToken:string;destination:NativePath;importSource:string;overlap:'require'|'reuse';overlapReason:string;keywordOverlap:'require'|'reuse';keywordReason:string;artifacts:ArtifactReceipt[];supplements:ExactDocument[];authorization:string}):string {
  ...
  if(value.artifacts.length>4096||value.supplements.length>4096||value.artifacts.some(item=>!uuid(item.receipt)))throw new Error('Prepared approval roster exceeds the supported bound or contains an invalid receipt.');
